...
export default function ContestList({
  ...
  contestsParams: contestType[],
  registeredContestsParams: number[],
  closable: boolean
}) {
  ...
  return (
    <div className="grid w-full gap-10 laptop:gap-2 desktop:gap-8 justify-evenly tablet:justify-evenly items-stretch pt-4" 
         style={{gridTemplateColumns: 'repeat(auto-fill, 21rem)'}}
    > 
      <AnimatePresence>
        {contests.map((contest) => (
          ...
          >
            ...
            <ContestCard 
              contest={contest} 
              onClose={removeContest} 
              registered={registeredContests.some(id => id === contest.id)} 
              onRegister={() => register(contest.id)} 
              closable={closable}  
            />
          </motion.div>
        ))}
