...
export default function SideNav(){
  ...
  return <div className='flex flex-col justify-start items-center h-full w-12 sm:w-48 bg-white shadow rounded-xl'>
    <div className='flex flex-col justify-start items-center grow text-gray-600 gap-2'>
      <div className="pb-12">
        <Link href="/">
          <div className='flex flex-row justify-begin items-center w-12 sm:w-48 text-2xl font-bold text-gray-900 p-4 gap-4'>
      ...
      </div>
      {
        nvaLinks.map((link) => (
          <SideNavButton url={link.url} type="text" key={link.text}>
            <Image src={link.icon} alt="" width={16} height={16}/> <span className="hidden sm:block">{link.text}</span>
          </SideNavButton>
        ))
      }
    ...
    <div className='flex flex-col justify-start items-center'>
        <div className="flex flex-row text-gray-600 w-12 sm:w-48 h-12 font-bold items-center gap-2 p-4 hover:bg-blue-100 cursor-pointer" onClick={handleLogout}>
...
const SideNavButton = ({children, url, type, onClick} : Readonly<{children:ReactNode|string, url : string, type : string, onClick?:() => void | Promise<void> | null}>) =>{
  ...
  return (
    <Link href={url} className="w-full">{
      type != 'icon' ? (
        <div className={`flex flex-row text-gray-600 w-12 sm:w-48 h-12 font-bold items-center gap-2 p-4 hover:bg-blue-100 ${pathname.startsWith(url) ? 'border-l-2 border-l-blue-600 text-blue-600' : ''}`}>
          {children}
        </div>
      ) : (
